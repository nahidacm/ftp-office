...
require_once($mosConfig_absolute_path."/plugins/system/legacy/dbtable.php");


class mosRealEstateManager_rent extends mosDBTable {
	/** @var int - Primary key */
	var $id = null;
	...
	*/
	function mosRealEstateManager_rent( &$db ) {
		$this->mosDBTable( '#__rem_rent', 'id', $db );
	}
	...
	function check() {
		...
		$this->_db->setQuery( "SELECT id FROM #__rem_rent "
			. " WHERE fk_houseid='$this->fk_houseid' AND rent_return = null"
		);						
  ...
  function getRentTo($userid){
	...
	function toXML(& $xmlDoc, $elementname="rent"){
