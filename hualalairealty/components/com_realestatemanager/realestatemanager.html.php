        ...
        if ($_REQUEST['m'] != 1) {
            ...
            for ($catInd = 0; $catInd < count($categories); $catInd++) {
                ...
                if ($countHouse) {
                    ...
                    ?>
                    <h1 id="article_title"><?php if ($categories[$catInd]->title != '') {
                        echo $categories[$catInd]->title;
                    } else {
                        echo $cat_Name[0]->title;
                    } ?></h1> 
                    <br />
                    <?php
                    for ($i = 0; $i < $numRows; $i++) {
                                ...
                                <a href="<?php echo sefRelToAbs($link); ?>" class="category<?php echo $params->get('pageclass_sfx'); ?>">
                                    ...
                                    if ($imageURL != '') {
                                        ...
                                        if (array_key_exists('filename', $file_pth))
                                            ...
                                            $file_name = $file_name = substr($imageURL, 0, strlen($imageURL) - strlen($file_pth['extension']) - 1);
                                        ?>
                        ...
                        ?>  
                            ...
                            <blockquote>
                        <?php if ($pcatID != '51') {
                            if ($_REQUEST['catid'] != '51') {
                                echo $row->bedrooms; ?> Bedrooms<br /><?php }
                        } ?>
                        <?php echo $row->price; ?>
                            </blockquote>
                        </div>
...
////////////Adding google map
        ...
        <script type="text/javascript">
            ...
            function initialize() {
                if (GBrowserIsCompatible()) {
                    ...
                    map.setCenter(new GLatLng(<?php if ($house->hlatitude) echo $house->hlatitude;
        else echo 0; ?>,
        <?php if ($house->hlongitude) echo $house->hlongitude;
        else echo 0; ?>),
        <?php if ($house->map_zoom) echo $house->map_zoom;
        else echo 1; ?>);
        ...
        if ($_REQUEST['m'] != 1) {
            ...
            <?php } if ($house->lanai != '') { ?>
                                    <blockquote><p><?php if ($house->catid == '54') {
                    echo "Covered Lanai Area";
                } else {
                    echo "Lanai Area";
                } ?><?php //echo _REALESTATE_MANAGER_LABEL_LANAI;?></p><abbr><?php echo $house->lanai; ?></abbr></blockquote>
                                    <?php
                                    }
            ...
            <?php } if ($house->features_list_link != '') { ?>
                                        <p>									
                                            <a href="<?php echo $house->features_list_link; ?>" target="blank">Features List</a>                                
                                        </p>
            <?php } if ($house->site_plan != '') { ?>
                                        <p>									
                                            <a href="<?php echo $house->site_plan; ?>" target="blank">Site Plan</a>                                
                                        </p>
            <?php } if ($house->plot_map_link != '') { ?>
                                        <p>									
                                            <a href="<?php echo $house->plot_map_link; ?>" target="blank">Plot Map</a>                                
                                        </p>
                    ...
                    <tr>
                        <td colspan="3">&nbsp;
                                        ...
                                        if ($params->get('show_inputreviews')) {
                                            ?>
                    <!--***********   begin add for show/hiden button "Add review"   ***********************-->
                    <div id ="button_hidden_review" style="<?php if (isset($_REQUEST['err_msg'])) {
                                                echo 'display:none';
                                            } ?>">
                        <br /> 
    ...
    function showInsertSubCategory($id, $cat_all, $params, $tabclass, $Itemid, $deep) {
        ...
        for ($i = 0; $i < count($cat_all); $i++) {
            if (($id == $cat_all[$i]->parent_id) && ($cat_all[$i]->display == 1)) {
                ...
                <?php if ($cat_all[$i]->houses == '') echo "0";
                else echo $cat_all[$i]->houses; ?>
                        </td>
                    </tr>
                </table>
                        <?php
...
//--------------------------------------------------------------------------------------------------
        ...
        <form action="<?php
        if ($catid == 0)
            echo sefRelToAbs("index.php?option=com_realestatemanager&amp;Itemid=" . $Itemid);
        else if ($_REQUEST['where'] == 2)
            echo sefRelToAbs("index.php?option=com_realestatemanager&amp;task=showCategory&amp;catid=" . $catid . "&amp;Itemid=" . $Itemid);
        else
            echo sefRelToAbs("index.php?option=com_realestatemanager&amp;Itemid=" . $Itemid);
