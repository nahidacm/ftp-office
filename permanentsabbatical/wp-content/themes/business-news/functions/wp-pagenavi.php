...
function pagenavi_menu() {
	if (function_exists('add_options_page')) {
		add_options_page(__('PageNavi', 'weblionmedia'), __('PageNavi', 'weblionmedia'), 'manage_options', 'wp-pagenavi/pagenavi-options.php') ;
	}
}*/
		...
		if($max_page > 1 || intval($pagenavi_options['always_show']) == 1) {
			...
			switch(intval($pagenavi_options['style'])) {
				case 1:
					...
					if ($start_page != $paged) {
						$output1 .= '<a href="'.esc_url(get_pagenum_link($paged-1)).'" class="prev">'.__('Previous','weblionmedia').'</a>';
					} else {
					...
					if ($end_page > $current_page_text) {
						$output1 .= '<a href="'.esc_url(get_pagenum_link($current_page_text+1)).'" class="next">'.__('Next','weblionmedia').'</a>';
					} else {
...
### Function: Page Navigation: Boxed Style Paging
function wp_pagenavi_recent_news($before = '', $after = '') {
	global $wpdb, $wp_query;
	...
	if (!is_single()) {
		...
		if($max_page > 1 || intval($pagenavi_options['always_show']) == 1) {
			...
			if ($news_page_id) { $news_final_url = $news_page_link; } else { $news_page_link = '#wasnt-set-template-all-page'; }
			...
			switch(intval($pagenavi_options['style'])) {
				case 1:
					...
					if ($start_page != $paged) {
						$output1 .= '<a href="'.$news_page_link.'" class="prev">'.__('Previous','weblionmedia').'</a>';
					} else {
						//$output1 .= '<div class="prev_first"></div>';
						$output1 .= '<a href="'.$news_page_link.'" class="prev">'.__('Previous','weblionmedia').'</a>';
					}
					...
					for($i = $start_page; $i  <= $end_page; $i++) {
						...
						if($i == $paged) {
							$current_page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $pagenavi_options['current_text']);
							if ($i == $end_page) {
								$output2 .= '<li class="current"><a href="'.$news_page_link.'" title="'.$page_text.'">'.$current_page_text.'</a></li>';
							} else {
								$output2 .= '<li class="current"><a href="'.$news_page_link.'" title="'.$page_text.'">'.$current_page_text.'</a></li>';
							}
						} else {
							$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $pagenavi_options['page_text']);
							if ($i == $end_page) {
								$output2 .= '<li><a href="'.$news_page_link.'" title="'.$page_text.'">'.$page_text.'</a></li>';
							} else {
								$output2 .= '<li><a href="'.$news_page_link.'" title="'.$page_text.'">'.$page_text.'</a></li>';
							}
						}
					...
					if ($end_page > $current_page_text) {
						$output1 .= '<a href="'.$news_page_link.'" class="next">'.__('Next','weblionmedia').'</a>';
					} else {
						//$output1 .= '<div class="next_last"></div>';
						$output1 .= '<a href="'.$news_page_link.'" class="next">'.__('Next','weblionmedia').'</a>';
					}
...
function pagenavi_init() {
	...
	$pagenavi_options['pages_text'] = __('Page %CURRENT_PAGE% of %TOTAL_PAGES%','weblionmedia');
	$pagenavi_options['current_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['page_text'] = '%PAGE_NUMBER%';
	$pagenavi_options['first_text'] = __('First','weblionmedia');
	$pagenavi_options['last_text'] = __('Last','weblionmedia');
	$pagenavi_options['next_text'] = __('Next','weblionmedia');
	$pagenavi_options['prev_text'] = __('Prev','weblionmedia');
	$pagenavi_options['dotright_text'] = __('...','weblionmedia');
	$pagenavi_options['dotleft_text'] = __('...','weblionmedia');
