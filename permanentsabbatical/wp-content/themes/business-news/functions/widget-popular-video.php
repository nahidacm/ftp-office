 ...
 */
add_action( 'widgets_init', 'popular_video_load_widgets' );

/**
 * Register our widget.
 * 'popular_video_Widget' is the widget class used below.
 */
function popular_video_load_widgets() {
	register_widget( 'popular_video_Widget' );
}
...
class popular_video_Widget extends WP_Widget {
	 ...
	 * Widget setup.
	 */
	function popular_video_Widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'popular', 'description' => 'Popular Video Post.' );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'popular-widget' );

		/* Create the widget. */
		$this->WP_Widget( 'popular-widget', 'WLM - Popular Video Post', $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$sql = 'SELECT * FROM wp_term_taxonomy				LEFT JOIN wp_term_relationships on wp_term_relationships.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id				LEFT JOIN wp_posts on wp_posts.ID = wp_term_relationships.object_id				LEFT JOIN wp_postmeta on wp_postmeta.meta_key='."'post_views_count'".' AND wp_postmeta.post_id=wp_posts.ID				WHERE wp_term_taxonomy.term_id = ' . $post_id . '				ORDER BY CAST(meta_value as SIGNED) DESC LIMIT 1';
		...
		foreach ($posts as $post) {
			...
			$post_output = '				
				<div class="content">
					...
					<div class="media"><a href="'.$video_url.'" class="general_pic_hover play no_fx" data-rel="prettyPhoto" title="'.$post_title.'">'.$featured_project_thumb.'</a></div>
					<p><a href="'.get_permalink($post->ID).'">'.$post_title.'</a> <img src="'.get_template_directory_uri().'/images/icon_video.gif" alt=""></p>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => 'Popular Video.' );
