...
while (have_posts()) : the_post();
    ...
    setPostViews($this_post_id);//Increase post view count
    ...
    if ($custom_post_type == 'media') {
        ...
        if ($custom_post_type != 'media') {
                            ...
                            <ul class="slides">
                                ...
                                for ($i = 0; $i < count($images_array); $i++) {
                                    ...
                                    <li>
                                        ...
                                        <div class="caption"><p><b><?php _e('Photo', 'weblionmedia');
                    echo ($i + 1);
                                    ?>.</b> <?php the_title(); ?></p></div>
                                    </li>
                                    <?php
                                }
                                ?>									
            ...
            if (( function_exists('get_post_format') && '' == get_post_format($post->ID))) :
                if ($single_image_preview) {
                    ?><div class="f_pic"><a href="#"><img width="100%" src="<?php echo $single_image_preview; ?>"></a></div>
                <?php } ?>
    <?php endif; ?>
            ...
            <?php wp_link_pages('before=<div id="page-links"><span>' . __('Pages:', 'weblionmedia') . '</span>&after=</div>&link_before=<div>&link_after=</div>'); ?>								
                ...
                <div class="about_author">
                    ...
                    if ($curauth->first_name || $curauth->last_name)
                        $curauth_name = $curauth->first_name . ' ' . $curauth->last_name;
                    if (!$curauth->first_name || !$curauth->last_name)
                        $curauth_name = $curauth->nickname;
                    ?>							
                    <h4><?php _e('About the Author', 'weblionmedia'); ?></h4>
                    <p class="name"><a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>"><?php
                            if (!get_the_author_meta('first_name') && !get_the_author_meta('last_name')) {
                                the_author_posts_link();
                            } else {
                                echo get_the_author_meta('first_name') . ' ' . get_the_author_meta('last_name');
                    ...
                    <div class="block_author">
                        ...
                        <div class="bio">
                            ...
                            if ($user_facebook || $user_twitter || $user_fr || $user_vimeo || $user_rss) {
                                if ($user_facebook) {
                                    $user_facebook_output = '<li><a href="' . $user_facebook . '" class="fb">Facebook</a></li>';
                                }
                                if ($user_twitter) {
                                    $user_twitter_output = '<li><a href="' . $user_twitter . '" class="tw">Twitter</a></li>';
                                }
                                if ($user_fr) {
                                    $user_fr_output = '<li><a href="' . $user_fr . '" class="s_fr">Fr</a></li>';
                                }
                                if ($user_vimeo) {
                                    $user_vimeo_output = '<li><a href="' . $user_vimeo . '" class="vimeo">Vimeo</a></li>';
                                }
                                if ($user_rss) {
                                    $user_rss_output = '<li><a href="' . $user_rss . '" class="rss">RSS</a></li>';
                                }
    ...
    <?php
    if ((($custom_post_type == 'post') && (get_option($shortname . "_blog_block_post_social") == 'true')) || (($custom_post_type == 'news') && (get_option($shortname . "_news_block_post_social") == 'true')) || (($custom_post_type == 'media') && (get_option($shortname . "_media_block_post_social") == 'true'))) {
        ?>
            <div class="block_post_social">
                ...
                <section class="rating">
                    ...
                    <ul>
                        <?php if ((get_option($shortname . '_blog_post_views') == 'true') && ($custom_post_type == 'post')) { ?><li><span><?php echo getPostViews(get_the_ID()); ?></span><?php
                                if (getPostViews(get_the_ID()) > 1) {
                                    _e('views', 'weblionmedia');
                                } else {
                                    _e('view', 'weblionmedia');
                                }
                                ?></li><?php } ?>
                            <?php if ((get_option($shortname . '_news_post_views') == 'true') && ($custom_post_type == 'news')) { ?><li><span><?php echo getPostViews(get_the_ID()); ?></span><?php
                                if (getPostViews(get_the_ID()) > 1) {
                                    _e('views', 'weblionmedia');
                                } else {
                                    _e('view', 'weblionmedia');
                                ...
                                ?></li><?php } ?>
        <?php if ((get_option($shortname . '_media_post_views') == 'true') && ($custom_post_type == 'media')) { ?><li><span><?php echo getPostViews(get_the_ID()); ?></span><?php
            if (getPostViews(get_the_ID()) > 1) {
                _e('views', 'weblionmedia');
            } else {
                _e('view', 'weblionmedia');
        ...
        <?php post_comments_feed_link(__('Subscribe to comments', 'weblionmedia')); ?>
            ...
            <div class="block_related_posts">
                ...
                <div class="block_main_news">
                    ...
                    if ($custom_post_type == 'news') {
                        query_posts("post_type=news&order=desc&showposts=5");
                    }
